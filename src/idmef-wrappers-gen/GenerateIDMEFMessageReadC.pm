...
sub	header
{
     ...
     $self->output("
/* Auto-generated by the GenerateIDMEFMessageReadC package ($time) */
...
sub	struct_field_struct
{
    ...
    $self->output("
			case MSG_",  uc($field->{short_typename}), "_TAG", ": \{
				...
				if ( ! idmef_read_$field->{short_typename}(msg, tmp) )
					goto error;
...
");
    ...
    $self->output("
$struct->{typename} *idmef_read_$struct->{short_typename}(prelude_msg_t *msg, $struct->{typename} *$struct->{short_typename})
\{
	void *buf;
	uint8_t tag;
	uint32_t len;
