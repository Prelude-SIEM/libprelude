...
int	idmef_reference_to_string(idmef_reference_t *ptr, char *buffer, size_t size)
{
	int offset = 0;
...

	{
...

		{
			...
			MY_SNPRINTF(buffer, size, offset, "%s (%d)", idmef_reference_origin_to_string(i), i);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
        }
...
int	idmef_classification_to_string(idmef_classification_t *ptr, char *buffer, size_t size)
{
	...
	MY_CONCAT(to_string_uint64,
		  idmef_classification_get_ident(ptr),
		  buffer, size, offset);
	MY_SNPRINTF(buffer, size, offset, "\n");

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "text: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		while ( (elem = idmef_classification_get_next_reference(ptr, elem)) ) {
			...
			MY_SNPRINTF(buffer, size, offset, "reference(%d):\n", cnt);
			MY_CONCAT(idmef_reference_to_string, elem, buffer, size, offset);
...
int	idmef_user_id_to_string(idmef_user_id_t *ptr, char *buffer, size_t size)
{
	int offset = 0;
...

	{
...

		{
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "type: ");
			MY_SNPRINTF(buffer, size, offset, "%s (%d)", idmef_user_id_type_to_string(i), i);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
        }

	{
		...
		if ( field ) {
			...
			MY_CONCAT(to_string_uint32, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
...
int	idmef_user_to_string(idmef_user_t *ptr, char *buffer, size_t size)
{
...

	{
		...
		while ( (elem = idmef_user_get_next_user_id(ptr, elem)) ) {
			...
			MY_SNPRINTF(buffer, size, offset, "user_id(%d):\n", cnt);
			MY_CONCAT(idmef_user_id_to_string, elem, buffer, size, offset);
...
int	idmef_address_to_string(idmef_address_t *ptr, char *buffer, size_t size)
{
...

	{
		...
		if ( field ) {
			...
			MY_CONCAT(to_string_int32, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
...
int	idmef_web_service_to_string(idmef_web_service_t *ptr, char *buffer, size_t size)
{
	int offset = 0;
...
int	idmef_snmp_service_to_string(idmef_snmp_service_t *ptr, char *buffer, size_t size)
{
	int offset = 0;
...

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "security_name: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "context_name: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "context_engine_id: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
...
int	idmef_service_to_string(idmef_service_t *ptr, char *buffer, size_t size)
{
...

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "ip_version: ");
			MY_CONCAT(to_string_uint8, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
...

	{
		...
		if ( field ) {
			...
			MY_CONCAT(to_string_uint16, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "iana_protocol_number: ");
			MY_CONCAT(to_string_uint8, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "iana_protocol_name: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
	...
	case IDMEF_SERVICE_TYPE_WEB:
		MY_CONCAT(to_string_indent, indent, buffer, size, offset);
		MY_SNPRINTF(buffer, size, offset, ("web_service:\n"));
		MY_CONCAT(idmef_web_service_to_string,
			  idmef_service_get_web_service(ptr),
			  buffer, size, offset);
		break;
 
	case IDMEF_SERVICE_TYPE_SNMP:
		MY_CONCAT(to_string_indent, indent, buffer, size, offset);
		MY_SNPRINTF(buffer, size, offset, ("snmp_service:\n"));
		MY_CONCAT(idmef_snmp_service_to_string,
			  idmef_service_get_snmp_service(ptr),
			  buffer, size, offset);
		break;
...
int	idmef_source_to_string(idmef_source_t *ptr, char *buffer, size_t size)
{
...

	{
...

		{
			...
			MY_SNPRINTF(buffer, size, offset, "%s (%d)", idmef_source_spoofed_to_string(i), i);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
        }
...
int	idmef_file_access_to_string(idmef_file_access_t *ptr, char *buffer, size_t size)
{
...

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "user_id:\n");
			MY_CONCAT(idmef_user_id_to_string, field, buffer, size, offset);
		}
	}	
...
int	idmef_checksum_to_string(idmef_checksum_t *ptr, char *buffer, size_t size)
{
	int offset = 0;
...

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "value: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "key: ");
			MY_CONCAT(to_string_string, field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}

	{
		...
		{
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "algorithm: ");
			MY_SNPRINTF(buffer, size, offset, "%s (%d)", idmef_checksum_algorithm_to_string(i), i);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
        }
...
int	idmef_file_to_string(idmef_file_t *ptr, char *buffer, size_t size)
{
...

	{
		...
		if ( field ) {
			...
			MY_CONCAT(to_string_uint64, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
...

	{
		...
		while ( (elem = idmef_file_get_next_linkage(ptr, elem)) ) {
			...
			MY_SNPRINTF(buffer, size, offset, "linkage(%d):\n", cnt);
...

	{
		...
		while ( (elem = idmef_file_get_next_checksum(ptr, elem)) ) {
			...
			MY_SNPRINTF(buffer, size, offset, "checksum(%d):\n", cnt);
			MY_CONCAT(idmef_checksum_to_string, elem, buffer, size, offset);
...

	{
...

		{
			...
			MY_SNPRINTF(buffer, size, offset, "%s (%d)", idmef_file_fstype_to_string(*i), *i);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
        }
...
int	idmef_target_to_string(idmef_target_t *ptr, char *buffer, size_t size)
{
...

	{
...

		{
			...
			MY_SNPRINTF(buffer, size, offset, "%s (%d)", idmef_target_decoy_to_string(i), i);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
        }
...
int	idmef_confidence_to_string(idmef_confidence_t *ptr, char *buffer, size_t size)
{
...

	{
		...
		if ( field ) {
			...
			MY_CONCAT(to_string_float, *field, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "\n");
		}
	}
...
int	idmef_alert_to_string(idmef_alert_t *ptr, char *buffer, size_t size)
{
	...
	MY_CONCAT(to_string_indent, indent, buffer, size, offset);
	MY_SNPRINTF(buffer, size, offset, "messageid: ");
	MY_CONCAT(to_string_uint64,
		  idmef_alert_get_messageid(ptr),
		  buffer, size, offset);
	MY_SNPRINTF(buffer, size, offset, "\n");
...

	{
		...
		if ( field ) {
			MY_CONCAT(to_string_indent, indent, buffer, size, offset);
			MY_SNPRINTF(buffer, size, offset, "classification:\n");
			MY_CONCAT(idmef_classification_to_string, field, buffer, size, offset);
		}
	}	
...
int	idmef_heartbeat_to_string(idmef_heartbeat_t *ptr, char *buffer, size_t size)
{
	...
	MY_SNPRINTF(buffer, size, offset, "messageid: ");
	MY_CONCAT(to_string_uint64,
		  idmef_heartbeat_get_messageid(ptr),
		  buffer, size, offset);
	MY_SNPRINTF(buffer, size, offset, "\n");
