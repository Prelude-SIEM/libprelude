...
/* Auto-generated by the GenerateIDMEFTreeWrapC package (Fri Dec 12 17:17:39 2003) */
...
int idmef_additional_data_type_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "date_time" ) == 0)
		return date_time;
...
{
	return idmef_value_new_enum_numeric(idmef_type_additional_data_type, ptr->type);
}
...
int idmef_classification_origin_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "origin_unknown" ) == 0)
		return origin_unknown;

	if ( strcasecmp(name, "origin-unknown" ) == 0)
		return origin_unknown;
	...
	if ( strcasecmp(name, "vendor_specific" ) == 0)
...
{
	return idmef_value_new_enum_numeric(idmef_type_classification_origin, ptr->origin);
}
...
int idmef_userid_type_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "original_user" ) == 0)
		return original_user;

	if ( strcasecmp(name, "original-user" ) == 0)
		return original_user;

	if ( strcasecmp(name, "current_user" ) == 0)
		return current_user;

	if ( strcasecmp(name, "current-user" ) == 0)
		return current_user;

	if ( strcasecmp(name, "target_user" ) == 0)
		return target_user;

	if ( strcasecmp(name, "target-user" ) == 0)
		return target_user;

	if ( strcasecmp(name, "user_privs" ) == 0)
		return user_privs;

	if ( strcasecmp(name, "user-privs" ) == 0)
		return user_privs;

	if ( strcasecmp(name, "current_group" ) == 0)
		return current_group;

	if ( strcasecmp(name, "current-group" ) == 0)
		return current_group;

	if ( strcasecmp(name, "group_privs" ) == 0)
		return group_privs;

	if ( strcasecmp(name, "group-privs" ) == 0)
		return group_privs;

	if ( strcasecmp(name, "other_privs" ) == 0)
		return other_privs;
...
{
	return idmef_value_new_enum_numeric(idmef_type_userid_type, ptr->type);
}
...
int idmef_user_category_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "cat_unknown" ) == 0)
		return cat_unknown;

	if ( strcasecmp(name, "cat-unknown" ) == 0)
		return cat_unknown;
	...
	if ( strcasecmp(name, "os_device" ) == 0)
		return os_device;
...
idmef_user_t *idmef_user_new(void)
{
	...
	INIT_LIST_HEAD(&ret->userid_list);
	...
	switch ( child ) {
		...
		case 2: {
			...
			if ( n < 0 ) {
				...
				i = 0;
				list_for_each(tmp, &ptr->userid_list) {
			    		entry = list_entry(tmp, idmef_userid_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_user_destroy_internal(idmef_user_t *ptr)
{

	{
		...
		list_for_each_safe(tmp, n, &ptr->userid_list) {
			entry = list_entry(tmp, idmef_userid_t, list);
...
{
	return idmef_value_new_enum_numeric(idmef_type_user_category, ptr->category);
}
...
{
    	return list_get_next(object, &ptr->userid_list, idmef_userid_t, list);
}

void idmef_user_set_userid(idmef_user_t *ptr, idmef_userid_t *object)
{
	list_add_tail(&object->list, &ptr->userid_list);
}
...
idmef_value_t *idmef_user_get_userid_value(idmef_user_t *ptr)
{
	...
	list_for_each(tmp, &ptr->userid_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_userid);
		if ( ! val ) {
...
int idmef_address_category_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "addr_unknown" ) == 0)
		return addr_unknown;

	if ( strcasecmp(name, "addr-unknown" ) == 0)
		return addr_unknown;
	...
	if ( strcasecmp(name, "e_mail" ) == 0)
		return e_mail;

	if ( strcasecmp(name, "e-mail" ) == 0)
		return e_mail;

	if ( strcasecmp(name, "lotus_notes" ) == 0)
		return lotus_notes;
...
{
	return idmef_value_new_enum_numeric(idmef_type_address_category, ptr->category);
}
...
idmef_process_t *idmef_process_new(void)
{
	...
	INIT_LIST_HEAD(&ret->arg_list);
    

	
	INIT_LIST_HEAD(&ret->env_list);
	...
	switch ( child ) {
		...
		case 4: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->arg_list) {
			    		entry = list_entry(tmp, idmef_string_t, list);
					if ( i++ == n )
						return entry;
		...
		case 5: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->env_list) {
			    		entry = list_entry(tmp, idmef_string_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_process_destroy_internal(idmef_process_t *ptr)
{
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->arg_list) {
			entry = list_entry(tmp, idmef_string_t, list);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->env_list) {
			entry = list_entry(tmp, idmef_string_t, list);
			idmef_string_destroy(entry);
		}
	}
...
{
    	return list_get_next(object, &ptr->arg_list, idmef_string_t, list);
}

void idmef_process_set_arg(idmef_process_t *ptr, idmef_string_t *object)
{
	list_add_tail(&object->list, &ptr->arg_list);
}
...
{
    	return list_get_next(object, &ptr->env_list, idmef_string_t, list);
}

void idmef_process_set_env(idmef_process_t *ptr, idmef_string_t *object)
{
	list_add_tail(&object->list, &ptr->env_list);
}
...
int idmef_service_type_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "no_specific_service" ) == 0)
		return no_specific_service;

	if ( strcasecmp(name, "no-specific-service" ) == 0)
		return no_specific_service;

	if ( strcasecmp(name, "web_service" ) == 0)
		return web_service;

	if ( strcasecmp(name, "web-service" ) == 0)
		return web_service;

	if ( strcasecmp(name, "snmp_service" ) == 0)
		return snmp_service;

	if ( strcasecmp(name, "snmp-service" ) == 0)
		return snmp_service;
...
void *idmef_service_get_child(void *p, idmef_child_t child)
{
	...
	switch ( child ) {
		...
		case 6: 
			if ( ptr->type == web_service ) 
				return ptr->specific.web;
			return NULL;   	   

		case 7: 
			if ( ptr->type == snmp_service ) 
...
{
	return idmef_value_new_enum_numeric(idmef_type_service_type, ptr->type);
}

idmef_webservice_t *idmef_service_get_web(idmef_service_t *ptr)
{
	return (ptr->type == web_service) ? ptr->specific.web : NULL;
}
...
void idmef_service_set_web(idmef_service_t *ptr, idmef_webservice_t *web)
{
	...
	ptr->type = web_service;
}

idmef_value_t *idmef_service_get_web_value(idmef_service_t *ptr)
{
	return ((ptr->type == web_service) ?
		idmef_value_new_object(ptr->specific.web, idmef_type_webservice) :
		NULL);
}
...
{
	return (ptr->type == snmp_service) ? ptr->specific.snmp : NULL;
}
...
void idmef_service_set_snmp(idmef_service_t *ptr, idmef_snmpservice_t *snmp)
{
	...
	ptr->type = snmp_service;
}

idmef_value_t *idmef_service_get_snmp_value(idmef_service_t *ptr)
{
	return ((ptr->type == snmp_service) ?
		idmef_value_new_object(ptr->specific.snmp, idmef_type_snmpservice) :
		NULL);
}
...
int idmef_node_category_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "node_unknown" ) == 0)
		return node_unknown;

	if ( strcasecmp(name, "node-unknown" ) == 0)
		return node_unknown;
...
idmef_node_t *idmef_node_new(void)
{
	...
	INIT_LIST_HEAD(&ret->address_list);
	...
	switch ( child ) {
		...
		case 4: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->address_list) {
			    		entry = list_entry(tmp, idmef_address_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_node_destroy_internal(idmef_node_t *ptr)
{
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->address_list) {
			entry = list_entry(tmp, idmef_address_t, list);
...
{
	return idmef_value_new_enum_numeric(idmef_type_node_category, ptr->category);
}
...
{
    	return list_get_next(object, &ptr->address_list, idmef_address_t, list);
}

void idmef_node_set_address(idmef_node_t *ptr, idmef_address_t *object)
{
	list_add_tail(&object->list, &ptr->address_list);
}
...
idmef_value_t *idmef_node_get_address_value(idmef_node_t *ptr)
{
	...
	list_for_each(tmp, &ptr->address_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_address);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
int idmef_spoofed_to_numeric(const char *name)
{
...
const char *idmef_spoofed_to_string(int val)
{
	switch ( val ) {
...
idmef_spoofed_t idmef_source_get_spoofed(idmef_source_t *ptr)
{
	return ptr->spoofed;
}
...
{
	return idmef_value_new_enum_numeric(idmef_type_spoofed, ptr->spoofed);
}

void idmef_source_set_spoofed(idmef_source_t *ptr, idmef_spoofed_t spoofed)
{
	ptr->spoofed = spoofed;
}

idmef_spoofed_t *idmef_source_new_spoofed(idmef_source_t *ptr)
{
	return &ptr->spoofed;
}
...
{
	return idmef_value_new_object(ptr->node, idmef_type_node);
}
...
{
	return idmef_value_new_object(ptr->user, idmef_type_user);
}
...
{
	return idmef_value_new_object(ptr->process, idmef_type_process);
}
...
{
	return idmef_value_new_object(ptr->service, idmef_type_service);
}
...
idmef_file_access_t *idmef_file_access_new(void)
{
	...
	INIT_LIST_HEAD(&ret->permission_list);
	...
	switch ( child ) {
		...
		case 1: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->permission_list) {
			    		entry = list_entry(tmp, idmef_string_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_file_access_destroy_internal(idmef_file_access_t *ptr)
{
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->permission_list) {
			entry = list_entry(tmp, idmef_string_t, list);
			idmef_string_destroy(entry);
		}
	}
...
{
	return idmef_value_new_object(&ptr->userid, idmef_type_userid);
}
...
{
    	return list_get_next(object, &ptr->permission_list, idmef_string_t, list);
}

void idmef_file_access_set_permission(idmef_file_access_t *ptr, idmef_string_t *object)
{
	list_add_tail(&object->list, &ptr->permission_list);
}
...
idmef_file_t *idmef_file_new(void)
{
	...
	INIT_LIST_HEAD(&ret->file_access_list);
    

	
	INIT_LIST_HEAD(&ret->file_linkage_list);
	...
	switch ( child ) {
		...
		case 10: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->file_access_list) {
			    		entry = list_entry(tmp, idmef_file_access_t, list);
					if ( i++ == n )
						return entry;
		...
		case 11: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->file_linkage_list) {
			    		entry = list_entry(tmp, idmef_linkage_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_file_destroy_internal(idmef_file_t *ptr)
{
	...
	idmef_string_destroy_internal(&ptr->fstype);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->file_access_list) {
			entry = list_entry(tmp, idmef_file_access_t, list);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->file_linkage_list) {
			entry = list_entry(tmp, idmef_linkage_t, list);
...
{
	return idmef_value_new_enum_numeric(idmef_type_file_category, ptr->category);
}
...
idmef_string_t *idmef_file_get_fstype(idmef_file_t *ptr)
{
	return &ptr->fstype;
}
...
idmef_value_t *idmef_file_get_fstype_value(idmef_file_t *ptr)
{
	...
	value = idmef_value_new_string(&ptr->fstype);
	if ( ! value )
		return NULL;
...
void idmef_file_set_fstype(idmef_file_t *ptr, idmef_string_t *fstype)
{
	idmef_string_destroy_internal(&ptr->fstype);
	memcpy(&ptr->fstype, fstype, sizeof (ptr->fstype));
	free(fstype);
}

idmef_string_t *idmef_file_new_fstype(idmef_file_t *ptr)
{
	idmef_string_destroy_internal(&ptr->fstype);
...
{
    	return list_get_next(object, &ptr->file_access_list, idmef_file_access_t, list);
}

void idmef_file_set_file_access(idmef_file_t *ptr, idmef_file_access_t *object)
{
	list_add_tail(&object->list, &ptr->file_access_list);
}
...
idmef_value_t *idmef_file_get_file_access_value(idmef_file_t *ptr)
{
	...
	list_for_each(tmp, &ptr->file_access_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_file_access);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
    	return list_get_next(object, &ptr->file_linkage_list, idmef_linkage_t, list);
}

void idmef_file_set_file_linkage(idmef_file_t *ptr, idmef_linkage_t *object)
{
	list_add_tail(&object->list, &ptr->file_linkage_list);
}
...
idmef_value_t *idmef_file_get_file_linkage_value(idmef_file_t *ptr)
{
	...
	list_for_each(tmp, &ptr->file_linkage_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_linkage);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
	return idmef_value_new_object(ptr->inode, idmef_type_inode);
}
...
int idmef_linkage_category_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "hard_link" ) == 0)
		return hard_link;

	if ( strcasecmp(name, "hard-link" ) == 0)
		return hard_link;

	if ( strcasecmp(name, "mount_point" ) == 0)
		return mount_point;

	if ( strcasecmp(name, "mount-point" ) == 0)
		return mount_point;

	if ( strcasecmp(name, "reparse_point" ) == 0)
	...
	if ( strcasecmp(name, "symbolic_link" ) == 0)
...
{
	return idmef_value_new_enum_numeric(idmef_type_linkage_category, ptr->category);
}
...
{
	return idmef_value_new_object(ptr->file, idmef_type_file);
}
	...
	switch ( child ) {
		...
		case 7: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->file_list) {
			    		entry = list_entry(tmp, idmef_file_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_target_destroy_internal(idmef_target_t *ptr)
{
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->file_list) {
			entry = list_entry(tmp, idmef_file_t, list);
...
idmef_spoofed_t idmef_target_get_decoy(idmef_target_t *ptr)
{
	return ptr->decoy;
}
...
{
	return idmef_value_new_enum_numeric(idmef_type_spoofed, ptr->decoy);
}

void idmef_target_set_decoy(idmef_target_t *ptr, idmef_spoofed_t decoy)
{
	ptr->decoy = decoy;
}

idmef_spoofed_t *idmef_target_new_decoy(idmef_target_t *ptr)
{
	return &ptr->decoy;
}
...
{
    	return list_get_next(object, &ptr->file_list, idmef_file_t, list);
}

void idmef_target_set_file(idmef_target_t *ptr, idmef_file_t *object)
{
	list_add_tail(&object->list, &ptr->file_list);
}
...
idmef_value_t *idmef_target_get_file_value(idmef_target_t *ptr)
{
	...
	list_for_each(tmp, &ptr->file_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_file);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
int idmef_impact_severity_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "impact_low" ) == 0)
		return impact_low;

	if ( strcasecmp(name, "impact-low" ) == 0)
		return impact_low;

	if ( strcasecmp(name, "low" ) == 0)
		return impact_low;

	if ( strcasecmp(name, "impact_medium" ) == 0)
		return impact_medium;

	if ( strcasecmp(name, "impact-medium" ) == 0)
		return impact_medium;

	if ( strcasecmp(name, "medium" ) == 0)
		return impact_medium;

	if ( strcasecmp(name, "impact_high" ) == 0)
		return impact_high;

	if ( strcasecmp(name, "impact-high" ) == 0)
		return impact_high;
...
{
	return idmef_value_new_enum_numeric(idmef_type_impact_severity, ptr->severity);
}
...
{
	return idmef_value_new_enum_numeric(idmef_type_impact_completion, ptr->completion);
}
...
{
	return idmef_value_new_enum_numeric(idmef_type_impact_type, ptr->type);
}
...
int idmef_action_category_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "action_other" ) == 0)
		return action_other;

	if ( strcasecmp(name, "action-other" ) == 0)
		return action_other;

	if ( strcasecmp(name, "other" ) == 0)
		return action_other;

	if ( strcasecmp(name, "block_installed" ) == 0)
		return block_installed;

	if ( strcasecmp(name, "block-installed" ) == 0)
		return block_installed;

	if ( strcasecmp(name, "notification_sent" ) == 0)
		return notification_sent;

	if ( strcasecmp(name, "notification-sent" ) == 0)
		return notification_sent;

	if ( strcasecmp(name, "taken_offline" ) == 0)
...
{
	return idmef_value_new_enum_numeric(idmef_type_action_category, ptr->category);
}
...
{
	return idmef_value_new_enum_numeric(idmef_type_confidence_rating, ptr->rating);
}
	...
	switch ( child ) {
		...
		case 1: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->action_list) {
			    		entry = list_entry(tmp, idmef_action_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_assessment_destroy_internal(idmef_assessment_t *ptr)
{
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->action_list) {
			entry = list_entry(tmp, idmef_action_t, list);
...
{
	return idmef_value_new_object(ptr->impact, idmef_type_impact);
}
...
{
    	return list_get_next(object, &ptr->action_list, idmef_action_t, list);
}

void idmef_assessment_set_action(idmef_assessment_t *ptr, idmef_action_t *object)
{
	list_add_tail(&object->list, &ptr->action_list);
}
...
idmef_value_t *idmef_assessment_get_action_value(idmef_assessment_t *ptr)
{
	...
	list_for_each(tmp, &ptr->action_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_action);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
	return idmef_value_new_object(ptr->confidence, idmef_type_confidence);
}
...
idmef_tool_alert_t *idmef_tool_alert_new(void)
{
	...
	INIT_LIST_HEAD(&ret->alertident_list);
	...
	switch ( child ) {
		...
		case 2: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->alertident_list) {
			    		entry = list_entry(tmp, idmef_alertident_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_tool_alert_destroy_internal(idmef_tool_alert_t *ptr)
{
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->alertident_list) {
			entry = list_entry(tmp, idmef_alertident_t, list);
...
{
    	return list_get_next(object, &ptr->alertident_list, idmef_alertident_t, list);
}

void idmef_tool_alert_set_alertident(idmef_tool_alert_t *ptr, idmef_alertident_t *object)
{
	list_add_tail(&object->list, &ptr->alertident_list);
}
...
idmef_value_t *idmef_tool_alert_get_alertident_value(idmef_tool_alert_t *ptr)
{
	...
	list_for_each(tmp, &ptr->alertident_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_alertident);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
int idmef_alert_type_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "idmef_default" ) == 0)
		return idmef_default;

	if ( strcasecmp(name, "idmef-default" ) == 0)
		return idmef_default;

	if ( strcasecmp(name, "default" ) == 0)
		return idmef_default;

	if ( strcasecmp(name, "idmef_tool_alert" ) == 0)
		return idmef_tool_alert;

	if ( strcasecmp(name, "idmef-tool-alert" ) == 0)
		return idmef_tool_alert;

	if ( strcasecmp(name, "tool_alert" ) == 0)
		return idmef_tool_alert;

	if ( strcasecmp(name, "idmef_correlation_alert" ) == 0)
		return idmef_correlation_alert;

	if ( strcasecmp(name, "idmef-correlation-alert" ) == 0)
		return idmef_correlation_alert;

	if ( strcasecmp(name, "correlation_alert" ) == 0)
		return idmef_correlation_alert;

	if ( strcasecmp(name, "idmef_overflow_alert" ) == 0)
		return idmef_overflow_alert;

	if ( strcasecmp(name, "idmef-overflow-alert" ) == 0)
		return idmef_overflow_alert;

	if ( strcasecmp(name, "overflow_alert" ) == 0)
		return idmef_overflow_alert;
...
idmef_alert_t *idmef_alert_new(void)
{
	...
	INIT_LIST_HEAD(&ret->source_list);
    

	
	INIT_LIST_HEAD(&ret->target_list);
    

	
	INIT_LIST_HEAD(&ret->classification_list);
    

	
	INIT_LIST_HEAD(&ret->additional_data_list);
...
void *idmef_alert_get_child(void *p, idmef_child_t child)
{
	...
	switch ( child ) {
		...
		case 11: 
			if ( ptr->type == idmef_tool_alert ) 
				return ptr->detail.tool_alert;
			return NULL;   	   

		case 12: 
			if ( ptr->type == idmef_correlation_alert ) 
				return ptr->detail.correlation_alert;
			return NULL;   	   

		case 13: 
			if ( ptr->type == idmef_overflow_alert ) 
	...
	switch ( child ) {
		...
		case 6: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->source_list) {
			    		entry = list_entry(tmp, idmef_source_t, list);
					if ( i++ == n )
						return entry;
		...
		case 7: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->target_list) {
			    		entry = list_entry(tmp, idmef_target_t, list);
					if ( i++ == n )
						return entry;
		...
		case 8: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->classification_list) {
			    		entry = list_entry(tmp, idmef_classification_t, list);
					if ( i++ == n )
						return entry;
		...
		case 9: {
			...
			if ( n < 0 ) {
				...
				/* get n-th element of the list */
				i = 0;
				list_for_each(tmp, &ptr->additional_data_list) {
			    		entry = list_entry(tmp, idmef_additional_data_t, list);
					if ( i++ == n )
						return entry;
...
static void idmef_alert_destroy_internal(idmef_alert_t *ptr)
{
	...
	idmef_analyzer_destroy_internal(&ptr->analyzer);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->source_list) {
			entry = list_entry(tmp, idmef_source_t, list);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->target_list) {
			entry = list_entry(tmp, idmef_target_t, list);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->classification_list) {
			entry = list_entry(tmp, idmef_classification_t, list);
...

	{
		...
		list_for_each_safe(tmp, n, &ptr->additional_data_list) {
			entry = list_entry(tmp, idmef_additional_data_t, list);
...
{
	return idmef_value_new_object(ptr->assessment, idmef_type_assessment);
}
...
{
	return idmef_value_new_object(&ptr->analyzer, idmef_type_analyzer);
}

void idmef_alert_set_analyzer(idmef_alert_t *ptr, idmef_analyzer_t *analyzer)
{
	idmef_analyzer_destroy_internal(&ptr->analyzer);
	memcpy(&ptr->analyzer, analyzer, sizeof (ptr->analyzer));
	free(analyzer);
}
...
{
    	return list_get_next(object, &ptr->source_list, idmef_source_t, list);
}

void idmef_alert_set_source(idmef_alert_t *ptr, idmef_source_t *object)
{
	list_add_tail(&object->list, &ptr->source_list);
}
...
idmef_value_t *idmef_alert_get_source_value(idmef_alert_t *ptr)
{
	...
	list_for_each(tmp, &ptr->source_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_source);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
    	return list_get_next(object, &ptr->target_list, idmef_target_t, list);
}

void idmef_alert_set_target(idmef_alert_t *ptr, idmef_target_t *object)
{
	list_add_tail(&object->list, &ptr->target_list);
}
...
idmef_value_t *idmef_alert_get_target_value(idmef_alert_t *ptr)
{
	...
	list_for_each(tmp, &ptr->target_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_target);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
    	return list_get_next(object, &ptr->classification_list, idmef_classification_t, list);
}

void idmef_alert_set_classification(idmef_alert_t *ptr, idmef_classification_t *object)
{
	list_add_tail(&object->list, &ptr->classification_list);
}
...
idmef_value_t *idmef_alert_get_classification_value(idmef_alert_t *ptr)
{
	...
	list_for_each(tmp, &ptr->classification_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_classification);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
    	return list_get_next(object, &ptr->additional_data_list, idmef_additional_data_t, list);
}

void idmef_alert_set_additional_data(idmef_alert_t *ptr, idmef_additional_data_t *object)
{
	list_add_tail(&object->list, &ptr->additional_data_list);
}
...
idmef_value_t *idmef_alert_get_additional_data_value(idmef_alert_t *ptr)
{
	...
	list_for_each(tmp, &ptr->additional_data_list) {
		...
		val = idmef_value_new_object(entry, idmef_type_additional_data);
		if ( ! val ) {
			idmef_value_destroy(list_val);
			return NULL;
		}
...
{
	return idmef_value_new_enum_numeric(idmef_type_alert_type, ptr->type);
}

idmef_tool_alert_t *idmef_alert_get_tool_alert(idmef_alert_t *ptr)
{
	return (ptr->type == idmef_tool_alert) ? ptr->detail.tool_alert : NULL;
}
...
void idmef_alert_set_tool_alert(idmef_alert_t *ptr, idmef_tool_alert_t *tool_alert)
{
	...
	ptr->type = idmef_tool_alert;
}

idmef_value_t *idmef_alert_get_tool_alert_value(idmef_alert_t *ptr)
{
	return ((ptr->type == idmef_tool_alert) ?
		idmef_value_new_object(ptr->detail.tool_alert, idmef_type_tool_alert) :
		NULL);
}
...
{
	return (ptr->type == idmef_correlation_alert) ? ptr->detail.correlation_alert : NULL;
}
...
void idmef_alert_set_correlation_alert(idmef_alert_t *ptr, idmef_correlation_alert_t *correlation_alert)
{
	...
	ptr->type = idmef_correlation_alert;
}

idmef_value_t *idmef_alert_get_correlation_alert_value(idmef_alert_t *ptr)
{
	return ((ptr->type == idmef_correlation_alert) ?
		idmef_value_new_object(ptr->detail.correlation_alert, idmef_type_correlation_alert) :
		NULL);
}
...
{
	return (ptr->type == idmef_overflow_alert) ? ptr->detail.overflow_alert : NULL;
}
...
void idmef_alert_set_overflow_alert(idmef_alert_t *ptr, idmef_overflow_alert_t *overflow_alert)
{
	...
	ptr->type = idmef_overflow_alert;
}

idmef_value_t *idmef_alert_get_overflow_alert_value(idmef_alert_t *ptr)
{
	return ((ptr->type == idmef_overflow_alert) ?
		idmef_value_new_object(ptr->detail.overflow_alert, idmef_type_overflow_alert) :
		NULL);
}
...
int idmef_message_type_to_numeric(const char *name)
{
	...
	if ( strcasecmp(name, "idmef_alert_message" ) == 0)
		return idmef_alert_message;

	if ( strcasecmp(name, "idmef-alert-message" ) == 0)
		return idmef_alert_message;

	if ( strcasecmp(name, "idmef_heartbeat_message" ) == 0)
		return idmef_heartbeat_message;

	if ( strcasecmp(name, "idmef-heartbeat-message" ) == 0)
		return idmef_heartbeat_message;
...
void *idmef_message_get_child(void *p, idmef_child_t child)
{
	...
	switch ( child ) {
		...
		case 2: 
			if ( ptr->type == idmef_alert_message ) 
				return ptr->message.alert;
			return NULL;   	   

		case 3: 
			if ( ptr->type == idmef_heartbeat_message ) 
...
{
	return idmef_value_new_enum_numeric(idmef_type_message_type, ptr->type);
}

idmef_alert_t *idmef_message_get_alert(idmef_message_t *ptr)
{
	return (ptr->type == idmef_alert_message) ? ptr->message.alert : NULL;
}
...
void idmef_message_set_alert(idmef_message_t *ptr, idmef_alert_t *alert)
{
	...
	ptr->type = idmef_alert_message;
}

idmef_value_t *idmef_message_get_alert_value(idmef_message_t *ptr)
{
	return ((ptr->type == idmef_alert_message) ?
		idmef_value_new_object(ptr->message.alert, idmef_type_alert) :
		NULL);
}
...
{
	return (ptr->type == idmef_heartbeat_message) ? ptr->message.heartbeat : NULL;
}
...
void idmef_message_set_heartbeat(idmef_message_t *ptr, idmef_heartbeat_t *heartbeat)
{
	...
	ptr->type = idmef_heartbeat_message;
}

idmef_value_t *idmef_message_get_heartbeat_value(idmef_message_t *ptr)
{
	return ((ptr->type == idmef_heartbeat_message) ?
		idmef_value_new_object(ptr->message.heartbeat, idmef_type_heartbeat) :
		NULL);
}
