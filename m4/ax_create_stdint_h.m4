...
dnl Taken from the ethereal autoconf
dnl
AC_DEFUN([AC_PRELUDE_CHECK_64BIT_FORMAT],
[
...
AC_TRY_COMPILE([#include <stdint.h>],[int_least32_t v = 0; uint64_t z = 0;],
...
if test "$ac_stdint_h" = "stdint.h" ; then
 ...
 AC_MSG_RESULT([$ac_cv_header_stdint (found)])
fi

if test "_$ac_cv_header_stdint" != "_" ; then
  ac_cv_header_prix64=
  AC_MSG_CHECKING([for PRIx64 in $ac_cv_header_stdint])
  AC_EGREP_HEADER(PRIx64, $ac_cv_header_stdint,
    [
      AC_MSG_RESULT([yes])
      $ac_cv_header_prix64=$ac_cv_header_stdint
    ],
    [
      AC_MSG_RESULT([no])
    ])
...
if test "_$ac_cv_header_stdint" = "_" -o "_$ac_cv_header_prix64" = "_"; then # can not shortcircuit..
...
if test "_$ac_cv_header_prix64" == "_" ; then
  ac_cv_prix64_result="(not found)"
  for i in sys/types.h inttypes.h sys/inttypes.h $inttype_headers ; do
    found_prefix=
    AC_MSG_CHECKING([for PRIx64 in $i])
    AC_EGREP_CPP(yes,[#include <$i>
	  #ifdef PRIx64
	  yes
	  #endif
    ],
    [
      AC_MSG_RESULT(yes)
      ac_cv_header_prix64=$i
      ac_cv_prix64_result="(seen in $i)"
      break;
    ],
    [
      AC_MSG_RESULT(no)
...
if test "$ac_cv_sizeof_long" == "8" ; then
  ac_cv_64bit_type='unsigned long'
else
  ac_cv_64bit_type='unsigned long long'
fi

if test "_$ac_cv_header_prix64" != "_"; then
  AC_MSG_CHECKING([if the PRIx64 actually works])
  AC_RUN_IFELSE(
  [
    AC_LANG_SOURCE(
      [[
          #include <stdio.h>
          #include <$ac_cv_header_prix64>

          main()
          {
	    $ac_cv_64bit_type t = 1;
...
if test "_$ac_cv_header_prix64_works" != "_yes" ; then
AC_MSG_CHECKING([for a working PRIx64])
AC_MSG_RESULT([..])
AC_PRELUDE_CHECK_64BIT_FORMAT(ll, $ac_cv_header_stdint, $ac_cv_64bit_type,
  [ ac_prelude_pri64_prefix=ll ],
  [
    AC_PRELUDE_CHECK_64BIT_FORMAT(l, $ac_cv_header_stdint, $ac_cv_64bit_type,
      [ ac_prelude_pri64_prefix=l ],
      [
        AC_PRELUDE_CHECK_64BIT_FORMAT(L, $ac_cv_header_stdint, $ac_cv_64bit_type,
          [ ac_prelude_pri64_prefix=L ],
          [
            AC_PRELUDE_CHECK_64BIT_FORMAT(q, $ac_cv_header_stdint, $ac_cv_64bit_type,
              [ ac_prelude_pri64_prefix=q ],
              [
...
ac_cv_stdint_message="using gnu compiler "`$CC --version | head -n1` 
else
...
if test "_$ac_cv_header_prix64" != "_" ; then
  if test "_$ac_cv_header_prix64_works" == "_yes" ; then
    AC_MSG_RESULT([make use of $ac_cv_header_prix64 in $ac_stdint_h dnl
$ac_cv_prix64_result])
  else
    AC_MSG_RESULT([undef broken PRIx64 and redefine in $ac_stdint_h])
  fi
else
  AC_MSG_RESULT([no definition found. defining our own])
fi
...
if test "_$ac_cv_header_prix64" != "_" ; then
  echo "#define _STDINT_HAVE_PRIx64" "1" >>$ac_stdint
fi
...
/* if we don't have a working header, undef stuff */

#ifdef _STDINT_HAVE_PRIx64
  #ifndef _STDINT_HAVE_WORKING_PRIx64
    #undef PRId64
    #undef PRIi64
    #undef PRIo64
    #undef PRIx64
...
/* If we don't have a working defintion already, create one */

#ifndef _STDINT_HAVE_WORKING_PRI64_PREFIX
  #ifdef __PRELUDE_PRI64_PREFIX
...
ac_cv_header_prix64="$ac_cv_header_prix64"
ac_cv_header_prix64_works="$ac_cv_header_prix64_works"
ac_prelude_pri64_prefix="$ac_prelude_pri64_prefix"
